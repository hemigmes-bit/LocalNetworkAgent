...
$btnScan.Add_Click({
    Start-Process -FilePath "powershell.exe" -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -NoExit -File `"$projectPath\Scan-Network.ps1`"" -WindowStyle Hidden
})
...
$btnFiles.Add_Click({
    ...
    if ($ip) {
        Start-Process -FilePath "powershell.exe" -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -NoExit -File `"$projectPath\Explorer_Agent.ps1`" -ComputerName $ip" -WindowStyle Hidden
    }
})
...
$btnShell.Add_Click({
    $ip = Get-SelectedComputer
    if ($ip) {
        Start-Process -FilePath "powershell.exe" -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -NoExit -Command `"Import-Module '$projectPath\NetworkUtilsPublic.psm1'; Connect-RemoteComputer -ComputerName $ip`"" -WindowStyle Hidden
    }
})

$btnVoice.Add_Click({
    Start-Process -FilePath "powershell.exe" -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -NoExit -File `"$projectPath\VoiceControl.ps1`"" -WindowStyle Hidden
})

$btnIntercom.Add_Click({
    $ip = Get-SelectedComputer
    if ($ip) {
        Start-Process -FilePath "powershell.exe" -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -NoExit -File `"$projectPath\Intercom.ps1`" -ComputerName $ip" -WindowStyle Hidden
    }
})
...
$btnSpeak.Add_Click({
    $ip = Get-SelectedComputer
    if ($ip) {
        Start-Process -FilePath "powershell.exe" -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -NoExit -File `"$projectPath\Speak-Remote.ps1`" -ComputerName $ip" -WindowStyle Hidden
    }
})

$btnWoL.Add_Click({
    $config = Get-Content "$projectPath\network-config.json" | ConvertFrom-Json
    Start-Process -FilePath "powershell.exe" -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -Command `"Import-Module '$projectPath\NetworkUtilsPublic.psm1'; Send-WakeOnLan -MACAddress $($config.Core0MAC)`"" -WindowStyle Hidden
